...
# むしろ前後の文脈が増えて精度が上がるため、間隔と同じ長さにしておく。
CORRECT_WINDOW = 25.0         # 秒: 一度に聞き直す長さ
CORRECT_TIMEOUT = 25.0        # 秒: これを超えたら諦める（まれな暴走対策）
# 訂正係のスレッド数。実測では4スレッド9.1秒／2スレッド8.8秒とほぼ変わらないので、
# 2に絞って残りをVosk（速報字幕）に回した方が全体として速くなる。
CORRECT_CPU_THREADS = 2
...
# 0.5秒間隔ではCPUの28%を占有し、Vosk・Whisperと取り合いになって全部が遅くなった。
# 1.0秒に伸ばして14%に半減させた（2026-08-30実測）。
SOUND_EVENT_HOP = 1.0            # 秒: 何秒ごとに判定をやり直すか（短いほど反応が速いがCPUを食う）
