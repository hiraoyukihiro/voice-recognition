...
SILENCE_THRESHOLD = 0.0001  # マイク感度が低く、小さい声が0.003に届かず捨てられていたため引き下げ（reSpeaker使用時は0.003に）
OVERLAP_RMS_RATIO = 2.0

# --- 音量正規化設定 ---
# 固定倍率だと声の大小でWhisperの精度が変わるため、ピーク音量基準で正規化する
TARGET_PEAK = 0.7
MAX_GAIN = 100.0  # マイク感度が低いため上限を高めに設定（reSpeaker使用時は20.0に）
...
ENABLE_NOISE_REDUCTION = True  # 精度優先のため有効化（1〜2秒処理時間が増える）
